...
export default function TokenCard({
    ...
    return (
        <div className="bg-bg-surface rounded-card p-6 border border-white/5 flex flex-col justify-between hover:-translate-y-1 transition-transform group shadow-lg">
            ...
            <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-3">
                    {iconUrl ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-[#131128] flex items-center justify-center font-bold text-sm border border-white/10">
                            {symbol.charAt(0)}
                        </div>
                    )}
                ...
                <div className="flex flex-col items-end">
                    <span className="text-xs text-text-secondary font-bold uppercase mb-1">{symbol}</span>
            ...
            {/* Footer Info */}
            <div className="bg-[#131128] rounded-2xl p-4 border border-white/5 group-hover:border-primary-purple/30 transition-colors">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`text-xs font-bold ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
                        {isPositive ? '+' : ''}{change24h?.toFixed(2)}% {symbol}
                    </span>
                </div>
            ...
            {!hideBuyButton && (
                <button
                    ...
                    className="w-full mt-4 bg-[#6639E4] hover:bg-[#7b4dff] text-white font-bold py-3 px-4 rounded-xl transition-all active:scale-95 shadow-lg shadow-[#6639E4]/20 flex items-center justify-center gap-2"
                >
                    Comprar {name}
                </button>
            )}
